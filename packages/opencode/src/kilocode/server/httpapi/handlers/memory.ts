...
export const memoryHandlers = HttpApiBuilder.group(InstanceHttpApi, "memory", (handlers) =>
  Effect.gen(function* () {
    ...
    const zeroStatus = Effect.fn("MemoryHttpApi.zeroStatus")(function* (req: {
      query: typeof MemoryQuery.Type
    }) {
      const ctx = yield* InstanceState.context
      return yield* api(svc.zeroStatus({ ctx }))
    })

    const zeroRecall = Effect.fn("MemoryHttpApi.zeroRecall")(function* (req: {
      query: typeof MemoryQuery.Type
      payload: typeof MemoryZeroRecallPayload.Type
    }) {
      const ctx = yield* InstanceState.context
      const ev = yield* api(
        svc.zeroRecall({ ctx, query: req.payload.query, topK: req.payload.topK, sessionID: req.payload.sessionID }),
      )
      if (!ev) throw new Error("zero-mem disabled or empty")
      return {
        query: ev.query,
        route: ev.profile.route,
        answerType: ev.profile.answerType,
    ...
    return handlers
      ...
      .handle("purge", purge)
      .handle("zeroStatus", zeroStatus)
      .handle("zeroRecall", zeroRecall)
  }),
)
