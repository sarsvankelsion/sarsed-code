...
export const MemoryZeroRecallPayload = MemoryContract.ZeroRecallPayload
  ...
  .add(
    HttpApiGroup.make("memory")
      .add(
        ...
        HttpApiEndpoint.get("zeroStatus", MemoryPaths.zeroStatus, {
          query: MemoryQuery,
          success: described(MemoryContract.ZeroStatus, "Zero-Mem status"),
          error: MemoryErrors,
        }).annotateMerge(
          OpenApi.annotations({
            identifier: "memory.zeroStatus",
            summary: "Zero-Mem status",
            description:
              "Token-free substrate stats (traces/windows/episodes/entities) with zero LLM cost accounting.",
          }),
        ),
        HttpApiEndpoint.post("zeroRecall", MemoryPaths.zeroRecall, {
          query: MemoryQuery,
          payload: MemoryZeroRecallPayload,
          success: described(MemoryContract.ZeroRecall, "Zero-Mem recall"),
          error: MemoryErrors,
        }).annotateMerge(
          OpenApi.annotations({
            identifier: "memory.zeroRecall",
            summary: "Zero-Mem recall",
            description:
              "Dual-view (graph + hierarchy) recall over raw traces. Deterministic, zero LLM calls.",
          }),
        ),
      )
      .annotateMerge(
