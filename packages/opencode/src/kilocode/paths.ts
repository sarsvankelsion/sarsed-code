...
export namespace KilocodePaths {
  ...
  /** Global directories in user home: ~/.sarsed, ~/.kilocode, ~/.kilo */
  export function globalDirs(): string[] {
    return [path.join(home(), ".sarsed"), path.join(home(), ".kilocode"), path.join(home(), ".kilo")]
  }

  /**
   * Discover Kilo directories containing skills.
   * Returns parent directories (.sarsed/, .kilocode/ and .kilo/) for glob pattern "skills/[*]/SKILL.md".
   *
   * - Walks up from projectDir to worktreeRoot for .sarsed/, .kilocode/ and .kilo/
   * - Includes global ~/.sarsed/, ~/.kilocode/ and ~/.kilo/
  ...
  export async function skillDirectories(opts: {
    ...
    if (!opts.skipGlobalPaths) {
      // 1. Global ~/.sarsed/, ~/.kilocode/ and ~/.kilo/ (loaded first so project-level overrides)
    ...
    // 3. Walk up from project dir to worktree root for .sarsed/, .kilocode/ and .kilo/
    ...
    for (const target of [".sarsed", ".kilocode", ".kilo"] as const) {
