...
// Map "code" config key to "build" for canonical naming.
export function resolveKey(name: string): string {
  return name === "code" ? "build" : name
}

// Remap "code" → "build" in agent config entries for canonical naming in the config loop.
export function preprocessConfig<T>(agentConfig: Record<string, T>): Record<string, T> {
  ...
  for (const [key, value] of Object.entries(agentConfig)) {
    result[key === "code" ? "build" : key] = value
  }
  return result
}
...
export function patchAgents(
  ...
  // Retain "build" as canonical primary mode, and alias "code" as hidden fallback
  if (agents.build) {
    agents.build = {
      ...agents.build,
      name: "build",
  ...
  // Register Sarsed security validation mode
  if (!agents.sarsed) {
    agents.sarsed = {
      name: "sarsed",
      displayName: "Sarsed",
      description:
        "Sarsed security validation mode adapted from sarsed.md. Authorized security validation & automated remediation.",
      options: {},
      color: "#FF0000",
      permission: Permission.merge(
        defaults,
