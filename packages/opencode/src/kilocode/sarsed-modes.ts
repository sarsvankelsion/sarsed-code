...
`,
  ...
  export function getLegacyModePath(projectDir: string, slug: string): string {
    const safeSlug = sanitizeSlug(slug)
    return path.join(projectDir, ".sarsed", "agents", `${safeSlug}.md`)
  }
...
`
  ...
  /**
   * Delete a mode. Prevents deletion of default modes unless explicitly requested.
   */
