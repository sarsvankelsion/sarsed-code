...
import { MemoryMd, ZeroMemory } from "@kilocode/kilo-memory"
...
export namespace KilocodeSystemPrompt {
  ...
  export function environment(input: { ctx: InstanceContext; model: Provider.Model; editor?: EditorContext }) {
    return [
      [
        ...
        `  Project config: .sarsed/agents/*/AGENT.md, .sarsed/memory.md, .kilo/command/*.md, kilo.json, AGENTS.md. Put new agents in .sarsed/agents/<name>/AGENT.md.`,
  ...
  export function memoryBlocks(input: {
    ...
    return Effect.gen(function* () {
      ...
      // Read persistent MEMORY.md across working sessions (workspace .sarsed/memory.md or root MEMORY.md, and global)
      const memoryMd = yield* Effect.tryPromise(() => MemoryMd.read(input.ctx.directory)).pipe(
        Effect.orElseSucceed(() => undefined),
      )
      const globalMemoryMd = yield* Effect.tryPromise(() => MemoryMd.readGlobal()).pipe(
        Effect.orElseSucceed(() => undefined),
      )

      const mdBlocks: string[] = []
      // Zero-Mem token-free evidence: latest raw traces rendered deterministically
      // (no LLM call). Best-effort — never breaks prompt assembly.
      const zeroEv = yield* Effect.tryPromise(() =>
        ZeroMemory.recall({ root: MemoryPaths.root(input.ctx), query: "recent session continuity", topK: 3 }),
      ).pipe(Effect.orElseSucceed(() => undefined))
      if (zeroEv && zeroEv.items.length > 0 && zeroEv.cost.llmCalls === 0) {
        mdBlocks.push(ZeroMemory.renderBlock(zeroEv, 1500))
      }
      if (memoryMd?.content?.trim()) {
        mdBlocks.push(
          `<memory_md scope="workspace" path="${memoryMd.path}">\n${memoryMd.content.trim()}\n</memory_md>`,
        )
      }
      if (globalMemoryMd?.trim()) {
        mdBlocks.push(
          `<memory_md scope="global">\n${globalMemoryMd.trim()}\n</memory_md>`,
        )
      }

      // Emit the memory guidance once per prompt, not repeated per injected block.
      const guidance = [
        "The following Sarsed memory blocks and MEMORY.md context are saved project memory from previous sessions. You do have this prior-session context; never claim you lack memory of earlier work here while these blocks are present.",
        "Cross-Session Recall & Continuity: When the user starts a new session or asks where things stopped, what was happening, or wants to continue, immediately recall active tasks, architecture, and user conventions from the <memory_md> block without asking them to repeat themselves.",
        ...
        "When the user asks about prior work, where things stopped, what was happening, or wants to continue — however they phrase it — answer directly from latest_session_digest, the newest relevant session_digest record, or MEMORY.md.",
        ...
        "When the user explicitly asks you to remember, save, correct, update, or forget project memory, call kilo_memory_save and sync to MEMORY.md.",
      ...
      const allBlocks = [...blocks.map((block) => block.text.trim()), ...mdBlocks]
