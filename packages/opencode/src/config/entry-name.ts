...
export function configEntryNameFromPath(relativePath: string, prefixes: string[]) {
  let candidate = stripPrefix(relativePath, prefixes) ?? path.basename(relativePath)
  const ext = path.extname(candidate)
  candidate = ext.length ? candidate.slice(0, -ext.length) : candidate
  const normalized = candidate.replaceAll("\\", "/")
  if (normalized.toLowerCase().endsWith("/agent")) {
    return normalized.slice(0, -"/agent".length)
  }
  return candidate
}
