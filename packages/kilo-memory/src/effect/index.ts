...
import { ZeroMemory } from "../zero/facade"
...
export namespace KiloMemory {
  ...
  /** Zero-Mem: raw trace ingest + dual-view recall + status. Token-free (0 LLM calls). */
  export async function zeroIngest(input: Input & { sessionID: string; user: string; assistant: string; time?: number; recalledMemory?: boolean }) {
    const dir = await prepare(input)
    return ZeroMemory.ingestTurn({ root: dir, sessionID: input.sessionID, user: input.user, assistant: input.assistant, time: input.time, recalledMemory: input.recalledMemory })
  }

  export async function zeroRecall(input: Input & { query: string; topK?: number; sessionID?: string }) {
    const dir = await prepare(input)
    return ZeroMemory.recall({ root: dir, query: input.query, topK: input.topK, sessionID: input.sessionID ?? sessionIDOf(input) })
  }

  export async function zeroStatus(input: Input) {
    return ZeroMemory.status(await prepare(input))
  }

  function sessionIDOf(input: Input): string | undefined {
