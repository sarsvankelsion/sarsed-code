...
export namespace MemoryService {
  ...
  export interface Interface {
    ...
    readonly zeroIngest: (
      input: KiloMemory.Input & { sessionID: string; user: string; assistant: string; time?: number; recalledMemory?: boolean },
    ) => Effect.Effect<Awaited<ReturnType<typeof KiloMemory.zeroIngest>>, Failure>
    readonly zeroRecall: (
      input: KiloMemory.Input & { query: string; topK?: number; sessionID?: string },
    ) => Effect.Effect<Awaited<ReturnType<typeof KiloMemory.zeroRecall>>, Failure>
    readonly zeroStatus: (
      input: KiloMemory.Input,
    ) => Effect.Effect<Awaited<ReturnType<typeof KiloMemory.zeroStatus>>, Failure>
  ...
  export function make() {
    ...
    return Service.of({
      ...
      zeroIngest: (input) => bridge(() => KiloMemory.zeroIngest(input)),
      zeroRecall: (input) => bridge(() => KiloMemory.zeroRecall(input)),
      zeroStatus: (input) => bridge(() => KiloMemory.zeroStatus(input)),
