...
export namespace MemoryContract {
  ...
  export const ZeroStatus = Schema.Struct({
    root: Schema.String,
    enabled: Schema.Boolean,
    traces: Schema.Finite,
    windows: Schema.Finite,
    episodes: Schema.Finite,
    entities: Schema.Finite,
    encoderOps: Schema.Finite,
    llmCalls: Schema.Literal(0),
    llmTokens: Schema.Literal(0),
    evidenceTokens: Schema.Finite,
  })

  export const ZeroRecallPayload = Schema.Struct({
    query: PayloadText,
    topK: Schema.optional(Schema.Finite),
    sessionID: Schema.optional(PayloadSessionID),
  })

  export const ZeroRecallItem = Schema.Struct({
    traceId: Schema.String,
    sessionId: Schema.String,
    time: Schema.Finite,
    text: Schema.String,
    score: Schema.Finite,
    views: Schema.Array(Schema.Literals(["graph", "hierarchy"])),
    provenance: Schema.Literals(["trace", "md"]),
  })

  export const ZeroRecall = Schema.Struct({
    query: Schema.String,
    route: Schema.Literals(["relational", "local"]),
    answerType: Schema.String,
    items: Schema.Array(ZeroRecallItem),
    llmCalls: Schema.Literal(0),
    llmTokens: Schema.Literal(0),
    encoderOps: Schema.Finite,
  })

  export const ZeroQuery = Schema.Struct({
    query: Schema.String,
    topK: Schema.optional(Schema.Finite),
  })
  ...
  export const Paths = {
    ...
    forget: `${root}/forget`,
    purge: `${root}/purge`,
    zeroStatus: `${root}/zero/status`,
    zeroRecall: `${root}/zero/recall`,
  } as const
