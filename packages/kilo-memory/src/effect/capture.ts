...
export namespace MemoryCapture {
  export const turn = Effect.fn("MemoryCapture.turn")(function* (input: {
    ...
    if (!state.enabled || !state.capture.turnClose) {
      // Zero-Mem still ingests raw traces even when the typed pipeline is off:
      // the provenance substrate must not lose history. Best-effort, zero LLM cost.
      const probe = yield* input.session
        .readTurn({ sessionID: input.sessionID, window: MESSAGE_WINDOW })
        .pipe(Effect.catchCause(() => Effect.succeed(undefined)))
      if (probe) {
        yield* Effect.promise(() =>
          ZeroMemory.ingestTurn({
            root,
            sessionID: input.sessionID,
            user: probe.user,
            assistant: probe.assistant,
            recalledMemory: probe.recalledMemory,
          }),
        ).pipe(Effect.orElseSucceed(() => undefined))
      }
      return yield* skip("disabled")
    }
    const now = Date.now()
    ...
    // Zero-Mem ingest on the enabled path too: every completed turn lands in the
    // token-free substrate first; the typed pipeline below stays untouched.
    yield* Effect.promise(() =>
      ZeroMemory.ingestTurn({ root, sessionID: input.sessionID, user, assistant, recalledMemory: view.recalledMemory }),
    ).pipe(Effect.orElseSucceed(() => undefined))
