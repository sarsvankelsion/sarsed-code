...
export namespace MemoryRecall {
  ...
  export async function search(input: {
    ...
    // Zero-Mem fan-out: token-free substrate hits merge with typed+digest before
    // scoring. Best-effort — zero recall must never break classic recall.
    let zeroItems: Hit[] = []
    try {
      const { ZeroMemory } = await import("../zero/facade")
      zeroItems = await ZeroMemory.searchHits({ root: input.root, query, limit })
    } catch {
      zeroItems = []
    }
    ...
    const keys = MemoryTopics.expand(
      MemoryShared.terms(query, { drop: noise([...typedItems, ...digestItems, ...zeroItems]) }),
    )
    const hits = dedupe({
      hits: select({ hits: [...typedItems, ...digestItems, ...zeroItems], keys, limit, force: input.force }),
      query,
    })
    if (hits.length === 0) return
