...
export namespace MemoryTool {
  ...
  function recallSearch(input: Recall, live: Live, query: string, mode: MemoryRecall.Mode) {
    return Effect.gen(function* () {
      ...
      if (hits.length === 0) {
        return {
          title: `Sarsed memory ${input.params.mode}: no results`,
          output,
          metadata: { sources: [], count: 0 },
        } satisfies Result
      }
      ...
      return {
        title: `Sarsed memory ${input.params.mode}: ${hits.length} hit${hits.length === 1 ? "" : "s"}`,
        output,
  ...
  function write(input: Save, root: string) {
    return Effect.gen(function* () {
      ...
      // Live sync to project MEMORY.md
      yield* Effect.tryPromise(async () => {
        const { MemoryMd } = await import("./memory-md")
        const item = `- ${input.params.key ? `[${input.params.key}] ` : ""}${text}`
        if (input.params.action === "correct") {
          await MemoryMd.syncCompact(input.ctx.directory, { conventions: [item] })
        } else {
          await MemoryMd.syncCompact(input.ctx.directory, { decisions: [item] })
        }
  ...
  function title(input: { action: SaveParams["action"]; added: number; removed: number }) {
    if (input.action === "skip") return "Sarsed memory skipped: out of scope"
    if (input.action === "forget") return `Sarsed memory updated: ${input.removed} removed`
    if (input.added === 0) return "Sarsed memory unchanged"
    if (input.action === "correct")
      return `Sarsed memory correction saved: ${input.added} op${input.added === 1 ? "" : "s"}`
    return `Sarsed memory saved: ${input.added} op${input.added === 1 ? "" : "s"}`
  }
