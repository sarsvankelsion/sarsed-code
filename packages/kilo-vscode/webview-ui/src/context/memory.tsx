...
import type { MemoryStatusResponse, MemoryZeroStatusResponses } from "@kilocode/sdk/v2"
import type { ExtensionMessage } from "../types/messages"

export type ZeroStatus = MemoryZeroStatusResponses["200"]

export interface MemoryContextValue {
  status: Accessor<MemoryStatusResponse | undefined>
  zero: Accessor<ZeroStatus | undefined>
  loading: Accessor<boolean>
  pending: Accessor<boolean>
  error: Accessor<string | undefined>
  enabled: Accessor<boolean>
  totalTokens: Accessor<number>
  zeroTraces: Accessor<number>
  refresh: () => void
  inspect: () => void
  enable: () => void
  disable: () => void
...
export const MemoryProvider: ParentComponent = (props) => {
  ...
  const [zero, setZero] = createSignal<ZeroStatus | undefined>()
  ...
  const loaded = (message: Extract<ExtensionMessage, { type: "memoryLoaded" }>) => {
    ...
    if ("zero" in message && message.zero) setZero(message.zero as ZeroStatus)
    setError(undefined)
  }
  ...
  const value: MemoryContextValue = {
    ...
    totalTokens: total,
    zeroTraces: createMemo(() => zero()?.traces ?? 0),
    refresh,
    inspect,
