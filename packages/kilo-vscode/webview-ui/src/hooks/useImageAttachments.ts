...
import { ACCEPTED_MEDIA_TYPES, isAcceptedMediaType, isDragLeavingComponent } from "./image-attachments-utils"
...
export function useImageAttachments() {
  ...
  const handlePaste = (event: ClipboardEvent) => {
    ...
    const mediaItems = items.filter((item) => item.kind === "file" && ACCEPTED_MEDIA_TYPES.includes(item.type))
  ...
  const handleDrop = (event: DragEvent) => {
    ...
    // Second: fall through to media file drops (images, videos, audio)
