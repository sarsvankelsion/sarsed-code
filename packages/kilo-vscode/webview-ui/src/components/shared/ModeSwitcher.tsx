...
export const ModeSwitcherBase: Component<ModeSwitcherBaseProps> = (props) => {
  ...
  const agents = () => {
    const seen = new Map<string, (typeof props.agents)[0]>()
    for (const a of props.agents) {
      if ((a as { deprecated?: boolean }).deprecated || a.name?.toLowerCase() === "orchestrator") continue
      const key = (a.name || "").trim().toLowerCase()
      if (!key) continue
      const existing = seen.get(key)
      if (!existing || (a.description?.length ?? 0) > (existing.description?.length ?? 0)) {
        seen.set(key, a)
      }
    }
  ...
  const typeahead = createTypeahead(() => agents().map(formatAgentLabel))

  function openSelected() {
    if (props.blocked) return
    const list = agents()
    const idx = list.findIndex((a) => a.name === props.value)
  ...
  const triggerLabel = () => {
    const agent = agents().find((a) => a.name === props.value)
