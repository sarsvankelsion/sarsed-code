...
import { MemorySelector } from "../shared/MemorySelector"
...
import { isAcceptedVideoType, isAcceptedAudioType } from "../../hooks/image-attachments-utils"
...
export const PromptInput: Component<PromptInputProps> = (props) => {
  ...
  const handlePaste = (e: ClipboardEvent) => {
    ...
    // Detect YouTube URL pasted as plain text and append a hint for the model.
    const pastedText = e.clipboardData?.getData("text/plain") ?? ""
    if (
      !e.defaultPrevented &&
      /(?:youtube\.com\/(?:watch\?v=|shorts\/|embed\/)|youtu\.be\/)([\w-]{11})/.test(pastedText.trim())
    ) {
      // Let the URL land in textarea normally; the hint is appended after a tick
      // so the default textarea insertion completes first.
      const ref = textareaRef
      requestAnimationFrame(() => {
        if (!ref) return
        const cur = ref.value
        // Only append if URL is at the end of the input and no hint yet
        if (cur.endsWith(pastedText.trim()) && !cur.includes("[YouTube video attached")) {
          const next = cur + "\n[YouTube video attached — please use the webfetch tool to fetch its transcript or page content]"
          ref.value = next
          setText(next)
          adjustHeight()
          syncHighlightScroll()
    ...
    >
      ...
      <Show when={imageAttach.images().length > 0}>
        <div class="image-attachments">
          <For each={imageAttach.images()}>
            {(img) => (
              <Show
                when={!isAcceptedVideoType(img.mime) && !isAcceptedAudioType(img.mime)}
                fallback={
                  <div class={`image-attachment media-attachment media-attachment--${isAcceptedVideoType(img.mime) ? "video" : "audio"}`}>
                    <div
                      class="media-attachment-badge"
                      title={img.filename}
                    >
                      <span class="media-attachment-icon">
                        {isAcceptedVideoType(img.mime) ? "🎬" : "🎵"}
                      </span>
                      <span class="media-attachment-name">{img.filename}</span>
                    </div>
                    <IconButton
                      icon="close-small"
                      variant="ghost"
      ...
      <div class="prompt-input-hint">
        <div class="prompt-input-hint-selectors">
          ...
          <MemorySelector blocked={props.blocked?.() ?? false} />
        </div>
