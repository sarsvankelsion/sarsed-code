...
export class KiloProviderMemory {
  ...
  private async load(sessionID?: string): Promise<void> {
    try {
      ...
      // Zero-Mem substrate stats ride along with the classic status payload.
      // Best-effort: older backends lack zeroStatus — classic memory still loads.
      const zero = await retry(() => api.zeroStatus({ directory }, { throwOnError: true }))
        .then((r) => r.data)
        .catch(() => undefined)
      const msg = {
        type: "memoryLoaded",
