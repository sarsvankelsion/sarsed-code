...
import { SarsedModes } from "./sarsed-modes"
...
export async function activate(context: vscode.ExtensionContext) {
  console.log("Sarsed Code extension is now active")
  shuttingDown = false

  // Seed default bundled modes (Build, Plan, Sarsed) if not existing
  if (vscode.workspace.workspaceFolders) {
    for (const folder of vscode.workspace.workspaceFolders) {
      void SarsedModes.seedDefaultModes(folder.uri.fsPath).catch((err) => {
        console.error("Failed to seed default Sarsed modes:", err)
      })
    }
  }

  // Seed global ~/.sarsed/agents so modes exist even without an open workspace
  try {
    const os = await import("os")
    void SarsedModes.seedDefaultModes(os.homedir()).catch((err) => {
      console.error("Failed to seed global Sarsed modes:", err)
    })
  } catch {}
  ...
  context.subscriptions.push(
    vscode.workspace.onDidChangeWorkspaceFolders((event) => {
      for (const folder of event.added) {
        void SarsedModes.seedDefaultModes(folder.uri.fsPath).catch(console.error)
      }
    }),
  )
