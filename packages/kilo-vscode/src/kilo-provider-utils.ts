...
export function filterVisibleAgents(agents: Agent[]): { visible: Agent[]; defaultAgent: string } {
  ...
  for (const a of agents) {
    ...
    // Filter out deprecated agents like orchestrator
    ...
    if (!existing) {
      ...
      // Deduplicate: Keep whichever definition is richer (custom AGENT.md over generic fallback)
      const existingLen = existing.description?.length ?? 0
      const newLen = a.description?.length ?? 0
  ...
  const defaultAgent =
    visible.find((a) => a.name?.toLowerCase() === "build")?.name ||
    (visible.length > 0 ? visible[0]!.name : "build")
